...
\paragraph{Step 1. Linearization at \textbackslash(q\^{}\textbackslash star\textbackslash)}}
...
\subsubsection{Step 4. Optimal \textbackslash(\textbackslash lambda\textbackslash) from the LQR sliding cost}
...
\subsubsection{Step 7. Closed-form controller at \textbackslash(t = 0\textbackslash)}
...
\subsubsection{Step 8. Lab vs. analytic \textbackslash(\textbackslash lambda\textbackslash)}
...
\subsubsection{\textbackslash(\textbackslash Gamma\textbackslash) too small}

Transients long; parameter estimate never converges during the task.

\subsubsection{\textbackslash(\textbackslash Gamma\textbackslash) too large}
...
\subsubsection{Step 7. \textbackslash(\textbackslash sigma\textbackslash)-modification (robust variant)}
...
\subsubsection{Step 2. Error variables at \textbackslash(t = 0\textbackslash)}
...
\subsubsection{Step 3. Control torque at \textbackslash(t = 0\textbackslash)}
...
\subsubsection{Step 4. Adaptation update at \textbackslash(t = 0\textbackslash)}
...
\subsubsection{Step 7. \textbackslash(\textbackslash sigma\textbackslash)-modification trade-off}
...
\subsection{Optimization formulation (of \textbackslash(\textbackslash Gamma, \textbackslash sigma\textbackslash))}
...
\subsection{Full derivation: why the robust term \textbackslash(-\textbackslash rho\textbackslash tanh(s/\textbackslash varepsilon)\textbackslash) works}
...
\subsubsection{Step 1. Compute the disturbance bound \textbackslash(D\textbackslash)}
...
\subsubsection{Step 4. Torque at \textbackslash(t = 0\textbackslash) for worst-case payload \textbackslash(\textbackslash Delta m = +0.3\textbackslash)}
...
\subsection{Link to LMI and \textbackslash(H\_\textbackslash infty\textbackslash) design}
...
\subsubsection{Step 4. \textbackslash(z\_2\textbackslash) dynamics}
...
\subsubsection{Step 2. Virtual-control derivative at \textbackslash(t = 0\textbackslash)}
...
\subsubsection{Step 3. Quadratic expansion around a nominal \textbackslash((\textbackslash bar x\_k,\textbackslash bar u\_k)\textbackslash)}
...
\subsubsection{Step 4. Building the \textbackslash(Q\textbackslash)-function second-order expansion}
...
\subsubsection{Step 5. Minimise over \textbackslash(\textbackslash delta u\textbackslash) (closed-form quadratic)}
...
\subsubsection{Step 1. Forward-Euler rollout with \textbackslash(\textbackslash bar u = 0\textbackslash)}
...
\subsubsection{Step 3. Discrete Jacobians at \textbackslash(k = 2\textbackslash)}
...
\subsubsection{Step 4. Backward pass from \textbackslash(k = 2\textbackslash)}
...
\subsubsection{Step 5. Feedforward and feedback gain at \textbackslash(k = 2\textbackslash)}
...
\subsubsection{Step 6. Forward pass with step \textbackslash(\textbackslash alpha = 1\textbackslash)}
