...
\topruleSymbol & Name & Dimension & Physical Meaning \\
\midrule\bottomrule\(q\) & Joint positions & \(n \times 1\) & Generalised coordinates (angles for revolute joints) \\
...
\topruleDesign Choice & \(\zeta\) & \(\omega_n\) & \(K_P\) & \(K_D\) & Behaviour \\
\midrule\bottomruleUnderdamped & 0.5 & 10 & 100 & 10 & Fast but oscillatory \\
...
% Simulation setup
t_span = 0:0.01:20;          % 20 seconds, 10 ms step
...
function dx = manipulator_dynamics(t, x, params, qd, dqd, ddqd)
    ...
    a_cmd = ddq_des + params.Kd*de + params.Kp*e;
    tau   = M * a_cmd + C * dq + G;
...
\begin{lstlisting}[language=Matlab]
% ===== Run ODE45 =====
[t, x] = ode45(@(t,x) manipulator_dynamics(t, x, params, qd, dqd, ddqd), ...
               t_span, x0);

% ===== Compute desired trajectory for plotting =====
qd_vals = zeros(length(t), 2);
for i = 1:length(t)
    qd_vals(i,:) = qd(t(i))';
end
...
plot(t, qd_vals(:,1), 'b--', 'LineWidth', 1.2);
plot(t, qd_vals(:,2), 'r--', 'LineWidth', 1.2);
xlabel('Time (s)'); ylabel('Angle (rad)');
title('Joint Angles: Actual vs Desired');
legend('q_1', 'q_2', 'q_{1,d}', 'q_{2,d}');
grid on;

% ===== Plot: Tracking Error =====
subplot(1,2,2);
e_vals = qd_vals - x(:,1:2);
plot(t, e_vals(:,1), 'b', 'LineWidth', 1.5); hold on;
plot(t, e_vals(:,2), 'r', 'LineWidth', 1.5);
xlabel('Time (s)'); ylabel('Error (rad)');
title('Tracking Error');
legend('e_1', 'e_2');
grid on;

sgtitle('Computed Torque Control — 2-DOF Manipulator');
\end{lstlisting}
...
\topruleController & Model Knowledge & Steady-State Error & Transient Error & Coupling Rejection \\
\midrule\bottomrulePD Only & None & Non-zero (gravity droop) & Large & Poor \\
...
\[ M\,\ddot{e} + (C + K_D^*)\,\dot{e} + K_P^*\,e = \underset{\text{\footnotesize \text{perturbation } d(t)}}{\underline{\Delta M\,\ddot{q}_d + \Delta C\,\dot{q} + \Delta G}} \]
...
params_true = params;
params_true.m2 = 2;    % True mass of link 2 is DOUBLE

% Controller still uses params (m2 = 1) for its model
% We need a modified dynamics function:

function dx = manipulator_robust_test(t, x, params_ctrl, params_true, qd, dqd, ddqd)
    q  = x(1:2);
    dq = x(3:4);
    ...
    % --- Controller uses ESTIMATED model (params_ctrl) ---
    [M_hat, C_hat, G_hat] = build_model(q, dq, params_ctrl);
    a_cmd = ddq_des + params_ctrl.Kd*de + params_ctrl.Kp*e;
    tau   = M_hat * a_cmd + C_hat * dq + G_hat;
    
    % --- Plant uses TRUE model (params_true) ---
    [M_true, C_true, G_true] = build_model(q, dq, params_true);
    ddq = M_true \ (tau - C_true*dq - G_true);
    
    dx = [dq; ddq];
end

function [M, C, G] = build_model(q, dq, p)
...
b_viscous = 0.5;      % Viscous friction coefficient
f_coulomb = 0.3;      % Coulomb friction magnitude

% Friction torque (not modelled by controller)
tau_friction = b_viscous * dq + f_coulomb * sign(dq);

% Modified acceleration (true plant sees friction)
ddq = M_true \ (tau - C_true*dq - G_true - tau_friction);
\end{lstlisting}
...
if t >= 10 && t <= 10.5
    tau_dist = [5; 3];   % Disturbance torque (N·m)
else
    tau_dist = [0; 0];
end

ddq = M_true \ (tau - C_true*dq - G_true + tau_dist);
\end{lstlisting}
...
\topruleScenario & Peak Error (rad) & Steady-State Error & Stability \\
\midrule\bottomrulePerfect model & ≈ 0.02 (transient only) & ≈ 0 & Asymptotically stable \\
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.92\textwidth,keepaspectratio]{images/week_01/manipulator_tracking.png}
    \caption{Manipulator joint-space tracking.}
\end{figure}

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.92\textwidth,keepaspectratio]{images/week_01/mobile_robot_tracking.png}
    \caption{Differential-drive mobile robot trajectory tracking.}
\end{figure}

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.92\textwidth,keepaspectratio]{images/week_01/performance_summary.png}
    \caption{Performance summary across the three robotic platforms.}
\end{figure}

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.92\textwidth,keepaspectratio]{images/week_01/quadrotor_tracking.png}
    \caption{Quadrotor position / attitude tracking.}
\end{figure}
...
\toprule& Column 1: Joint Angles & Column 2: Tracking Error \\
\midrule\bottomrule\textbf{Row 1} & PD Only --- actual vs desired & PD Only --- error \\
...
\topruleEquation & Description \\
\midrule\bottomrule\(M(q)\ddot{q} + C(q,\dot{q})\dot{q} + G(q) = \tau\) & Robot dynamics (Euler--Lagrange) \\
