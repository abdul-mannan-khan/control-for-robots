...
\midrule\bottomrule\texttt{/map} & nav\_msgs/OccupancyGrid & Latched & Static occupancy grid map of the environment \\
...
\topruleTopic & Message Type & Publisher & Purpose \\
\midrule\bottomrule\texttt{/map} & \texttt{nav\_msgs/OccupancyGrid} & Map Server & Static occupancy grid from SLAM. Values: 0 = free, 100 = occupied, -1 = unknown. \\
...
\topruleAspect & DWA & MPC \\
\midrule\bottomrulePrediction Horizon & \(N = 1\) (or short trajectory) & \(N \gg 1\) (10--50 steps) \\
...
\topruleAspect & Finite State Machine & Behavior Tree \\
\midrule\bottomruleStructure & States + transitions (graph) & Tree of nodes (hierarchical) \\
...
\topruleController & Navigation Role & Example Scenario \\
\midrule\bottomrule\textbf{PID} & Pure pursuit path follower & Basic line following \\
...
\topruleTask & File & Description & Control Connection \\
\midrule\bottomrule\textbf{Task 1} & \texttt{task1\_costmap.py} & Costmap generation with static, obstacle, and inflation layers. Implement \texttt{create\_static\_layer()}, \texttt{create\_obstacle\_layer()}, \texttt{inflate\_costmap()}, and \texttt{combine\_layers()}. & Constraint encoding for MPC/DWA \\
...
\topruleExercise & File & Subscribes & Publishes & Key Function \\
\midrule\bottomrule\textbf{Exercise 1: Costmap Generator} & \texttt{exercise1\_node.py} & \texttt{/map} (OccupancyGrid) & \texttt{/costmap} (OccupancyGrid) & \texttt{inflate()} --- Inflate obstacles using exponential decay: \texttt{cost\ =\ 100\ *\ exp(-scale\ *\ dist)} \\
...
\begin{tabular}{lllllll}
  ...
  \topruleMetric & DWA (Corridor) & MPC (Corridor) & DWA (Open) & MPC (Open) & DWA (Dynamic) & MPC (Dynamic) \\
  \midrule\bottomruleCompletion Time (s) & & & & & & \\
  Min Obstacle Dist (m) & & & & & & \\
  Path Length (m) & & & & & & \\
...
\midrule\bottomrule\texttt{/map} & nav\_msgs/OccupancyGrid & Bag & Static occupancy grid (latched) \\
