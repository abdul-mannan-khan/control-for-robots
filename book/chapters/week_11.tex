...
\chapter{ROS\,2 Navigation Control}
\labch{week_11}

\section{AutoNav Solutions --- Navigation Engineer Technical Challenge}\label{autonav-solutions-navigation-engineer-technical-challenge}}
...
\subsection{Company Brief: AutoNav Solutions}\label{company-brief-autonav-solutions}}
...
\paragraph{Week 10 Exam Recall \& Introduction}\label{week-10-exam-recall-introduction}}

Review quadrotor control exam questions. Introduce mobile robot navigation as the next control application domain.

0:20 -- 0:50

\paragraph{Part 1: Nav2 Architecture Overview}\label{part-1-nav2-architecture-overview}}

Global planner, local controller, costmaps, AMCL, behavior trees, recovery behaviors. Key ROS2 topics and message types.

0:50 -- 1:20

\paragraph{Part 2: DWA --- The Local Controller as Optimization}\label{part-2-dwa-the-local-controller-as-optimization}}
...
\paragraph{Part 3: AMCL --- State Estimation for Control}\label{part-3-amcl-state-estimation-for-control}}
...
\paragraph{Part 4: Costmaps as Constraints \& Part 5: Behavior Trees}\label{part-4-costmaps-as-constraints-part-5-behavior-trees}}

Costmap inflation as soft constraints. Behavior tree navigation orchestration.

2:10 -- 2:25

\paragraph{Part 6: Applying Control Theory to Navigation}\label{part-6-applying-control-theory-to-navigation}}

Mapping all 8 control methods from the course to mobile robot navigation scenarios.

2:25 -- 3:00

\paragraph{Hands-On Activities \& Lab Exercises}\label{hands-on-activities-lab-exercises}}

Configure Nav2 for TurtleBot3. Tune DWA. Compare DWA vs MPC. Waypoint navigation with recovery.

\subsection{Section 3: Part 1 --- The ROS2 Navigation Stack (Nav2)}\label{section-3-part-1-the-ros2-navigation-stack-nav2}}
...
\subsubsection{3.1 Nav2 Core Components}\label{nav2-core-components}}

\paragraph{Global Planner}\label{global-planner}}
...
\paragraph{Local Controller}\label{local-controller}}
...
\paragraph{Costmap 2D}\label{costmap-2d}}
...
\paragraph{AMCL Localization}\label{amcl-localization}}
...
\paragraph{Behavior Tree Navigator}\label{behavior-tree-navigator}}
...
\paragraph{Recovery Behaviors}\label{recovery-behaviors}}
...
\subsubsection{3.2 Nav2 Architecture Diagram}\label{nav2-architecture-diagram}}

\subsubsection{3.3 Key Nav2 ROS2 Topics}\label{key-nav2-ros2-topics}}
...
\subsection{Section 4: Part 2 --- DWA: The Local Controller as Optimization}\label{section-4-part-2-dwa-the-local-controller-as-optimization}}
...
\subsubsection{4.1 DWA Velocity Space}\label{dwa-velocity-space}}

A differential-drive robot is controlled by two inputs: linear velocity \textbackslash(v\textbackslash) and angular velocity \textbackslash(\textbackslash omega\textbackslash). DWA operates in this 2D velocity space:

\textbf{Velocity Space:} \textbackslash( (v, \textbackslash omega) \textbackslash) where \textbackslash( v \textbackslash in {[}v\_\{\textbackslash min\}, v\_\{\textbackslash max\}{]} \textbackslash), \textbackslash( \textbackslash omega \textbackslash in {[}\textbackslash omega\_\{\textbackslash min\}, \textbackslash omega\_\{\textbackslash max\}{]} \textbackslash)

\subsubsection{4.2 Dynamic Window Constraint}\label{dynamic-window-constraint}}

Not all velocities in the admissible range are physically reachable from the current state within one control interval \textbackslash(\textbackslash Delta t\textbackslash). The \textbf{dynamic window} restricts the search to reachable velocities given acceleration limits:

\textbackslash{[}
V\_d = \textbackslash\{ (v, \textbackslash omega) \textbackslash mid v \textbackslash in {[}v\_c - \textbackslash dot\{v\}\_\{\textbackslash max\}\textbackslash Delta t,\textbackslash; v\_c + \textbackslash dot\{v\}\_\{\textbackslash max\}\textbackslash Delta t{]}, \textbackslash;
\textbackslash omega \textbackslash in {[}\textbackslash omega\_c - \textbackslash dot\{\textbackslash omega\}\_\{\textbackslash max\}\textbackslash Delta t,\textbackslash; \textbackslash omega\_c + \textbackslash dot\{\textbackslash omega\}\_\{\textbackslash max\}\textbackslash Delta t{]} \textbackslash\}
\textbackslash{]}

where \textbackslash((v\_c, \textbackslash omega\_c)\textbackslash) is the current velocity and \textbackslash((\textbackslash dot\{v\}\_\{\textbackslash max\}, \textbackslash dot\{\textbackslash omega\}\_\{\textbackslash max\})\textbackslash) are the maximum accelerations.

The \textbf{admissible velocities} are further constrained to those that allow the robot to stop before hitting an obstacle:

\textbackslash{[}
V\_a = \textbackslash\{ (v, \textbackslash omega) \textbackslash mid v \textbackslash leq \textbackslash sqrt\{2 \textbackslash cdot \textbackslash text\{dist\}(v,\textbackslash omega) \textbackslash cdot \textbackslash dot\{v\}\_\{\textbackslash max\}\},\textbackslash;
\textbackslash omega \textbackslash leq \textbackslash sqrt\{2 \textbackslash cdot \textbackslash text\{dist\}(v,\textbackslash omega) \textbackslash cdot \textbackslash dot\{\textbackslash omega\}\_\{\textbackslash max\}\} \textbackslash\}
\textbackslash{]}

\subsubsection{4.3 DWA Objective Function}\label{dwa-objective-function}}

DWA selects the velocity pair \textbackslash((v\^{}*, \textbackslash omega\^{}*)\textbackslash) that maximises a weighted objective:

\textbackslash{[} G(v, \textbackslash omega) = \textbackslash alpha \textbackslash cdot \textbackslash text\{heading\}(v,\textbackslash omega) + \textbackslash beta \textbackslash cdot \textbackslash text\{clearance\}(v,\textbackslash omega) + \textbackslash gamma \textbackslash cdot \textbackslash text\{velocity\}(v,\textbackslash omega) \textbackslash{]}
...
\tightlist
\item
  \textbf{heading(\textbackslash(v, \textbackslash omega\textbackslash)):} Measures alignment of the predicted trajectory\textquotesingle s endpoint with the goal direction. Defined as \textbackslash(180\^{}\textbackslash circ - \textbar\textbackslash theta\_\{\textbackslash text\{goal\}\} - \textbackslash theta\_\{\textbackslash text\{endpoint\}\}\textbar\textbackslash), normalised to {[}0, 1{]}. Encourages the robot to face the goal.
\item
  \textbf{clearance(\textbackslash(v, \textbackslash omega\textbackslash)):} Minimum distance from the predicted trajectory to the nearest obstacle. Larger clearance = safer trajectory. Returns \textbackslash(-\textbackslash infty\textbackslash) for collision trajectories (hard constraint).
\item
  \textbf{velocity(\textbackslash(v, \textbackslash omega\textbackslash)):} Forward speed \textbackslash(v\textbackslash) of the candidate, normalised to {[}0, 1{]}. Encourages faster progress toward the goal rather than overly cautious creeping.
\end{itemize}

\subsubsection{4.4 Connection to MPC (Week 3)}\label{connection-to-mpc-week-3}}

\textbf{Key Insight:} DWA can be viewed as MPC with horizon \textbackslash(N = 1\textbackslash), state = robot pose, control = \textbackslash((v, \textbackslash omega)\textbackslash), cost = negative of heading + clearance + velocity, and constraints = dynamic window + collision-free. The modern Nav2 \textbf{MPPI} (Model Predictive Path Integral) controller IS full MPC with \textbackslash(N \textbackslash gg 1\textbackslash).

\textbf{DWA Cost (1-step optimisation):}

\textbackslash{[} \textbackslash min\_\{v, \textbackslash omega\} -\textbackslash bigl{[}\textbackslash alpha \textbackslash cdot h(v,\textbackslash omega) + \textbackslash beta \textbackslash cdot c(v,\textbackslash omega) + \textbackslash gamma \textbackslash cdot v\textbackslash bigr{]} \textbackslash quad \textbackslash text\{s.t.\} \textbackslash quad (v,\textbackslash omega) \textbackslash in V\_d \textbackslash cap V\_a \textbackslash{]}

\hfill\break

\textbf{MPC Cost (N-step optimisation, Week 3):}

\textbackslash{[} \textbackslash min\_\{\textbackslash mathbf\{u\}\_\{0:N-1\}\} \textbackslash sum\_\{k=0\}\^{}\{N-1\} \textbackslash bigl{[}\textbackslash mathbf\{x\}\_k\^{}T \textbackslash mathbf\{Q\} \textbackslash mathbf\{x\}\_k + \textbackslash mathbf\{u\}\_k\^{}T \textbackslash mathbf\{R\} \textbackslash mathbf\{u\}\_k\textbackslash bigr{]} + \textbackslash mathbf\{x\}\_N\^{}T \textbackslash mathbf\{P\} \textbackslash mathbf\{x\}\_N \textbackslash quad \textbackslash text\{s.t. model, constraints\} \textbackslash{]}
...
\bottomrule\noalign{}
\endlastfoot
Prediction Horizon & \textbackslash(N = 1\textbackslash) (or short trajectory) & \textbackslash(N \textbackslash gg 1\textbackslash) (10--50 steps) \\
Decision Variables & \textbackslash((v, \textbackslash omega)\textbackslash) --- 2 scalars & \textbackslash(\textbackslash mathbf\{u\}\_\{0:N-1\}\textbackslash) --- \textbackslash(2N\textbackslash) variables \\
...
\subsubsection{4.5 ROS2 Implementation --- Simplified DWA Local Planner Node}\label{ros2-implementation-simplified-dwa-local-planner-node}}

\#!/usr/bin/env python3
"""Simplified DWA local planner node for ROS2.
Subscribes to /odom and /plan, publishes /cmd\_vel.
...
super().\_\_init\_\_(\textquotesingle dwa\_controller\textquotesingle)
\# DWA parameters
self.max\_v = 0.5; self.min\_v = 0.0
self.max\_w = 1.0; self.max\_accel = 0.5; self.max\_w\_accel = 1.0
self.v\_res = 0.05; self.w\_res = 0.1
self.dt = 0.1; self.predict\_time = 2.0
self.alpha = 1.0; self.beta = 0.8; self.gamma = 0.5 \# heading, clearance, velocity
self.robot\_radius = 0.18
\# State
self.x, self.y, self.th = 0.0, 0.0, 0.0
self.cur\_v, self.cur\_w = 0.0, 0.0
self.goal = None; self.obstacles = {[}{]}
\# ROS2 interfaces
self.create\_subscription(Odometry, \textquotesingle/odom\textquotesingle, self.odom\_cb, 10)
self.create\_subscription(Path, \textquotesingle/plan\textquotesingle, self.path\_cb, 10)
self.create\_subscription(LaserScan, \textquotesingle/scan\textquotesingle, self.scan\_cb, 10)
self.cmd\_pub = self.create\_publisher(Twist, \textquotesingle/cmd\_vel\textquotesingle, 10)
self.timer = self.create\_timer(0.1, self.control\_loop) \# 10 Hz
self.get\_logger().info(\textquotesingle DWA Controller started\textquotesingle)
def odom\_cb(self, msg):
self.x = msg.pose.pose.position.x
self.y = msg.pose.pose.position.y
q = msg.pose.pose.orientation
self.th = 2.0 * math.atan2(q.z, q.w)
self.cur\_v = msg.twist.twist.linear.x
self.cur\_w = msg.twist.twist.angular.z
def path\_cb(self, msg):
if msg.poses:
p = msg.poses{[}-1{]}.pose.position \# Use final pose as goal
self.goal = (p.x, p.y)
def scan\_cb(self, msg):
obs = {[}{]}
for i, r in enumerate(msg.ranges):
if msg.range\_min \textless{} r \textless{} msg.range\_max:
angle = msg.angle\_min + i * msg.angle\_increment
ox = self.x + r * math.cos(self.th + angle)
oy = self.y + r * math.sin(self.th + angle)
obs.append((ox, oy))
self.obstacles = obs
def predict\_trajectory(self, v, w):
"""Forward-simulate trajectory for predict\_time seconds."""
x, y, th = self.x, self.y, self.th
traj = {[}(x, y){]}
for \_ in range(int(self.predict\_time / self.dt)):
x += v * math.cos(th) * self.dt
y += v * math.sin(th) * self.dt
th += w * self.dt
traj.append((x, y))
return traj, th
def score\_trajectory(self, traj, final\_th):
"""Score: alpha*heading + beta*clearance + gamma*velocity."""
if not self.goal:
return -float(\textquotesingle inf\textquotesingle)
ex, ey = traj{[}-1{]}
\# Heading: alignment with goal
goal\_angle = math.atan2(self.goal{[}1{]} - ey, self.goal{[}0{]} - ex)
heading = math.pi - abs(goal\_angle - final\_th)
heading = heading / math.pi \# normalise to {[}0, 1{]}
\# Clearance: min distance to obstacles
clearance = float(\textquotesingle inf\textquotesingle)
for (px, py) in traj:
for (ox, oy) in self.obstacles:
d = math.hypot(px - ox, py - oy)
clearance = min(clearance, d)
if clearance \textless{} self.robot\_radius:
return -float(\textquotesingle inf\textquotesingle) \# Collision!
...
return
if math.hypot(self.x - self.goal{[}0{]}, self.y - self.goal{[}1{]}) \textless{} 0.2:
self.cmd\_pub.publish(Twist()); return \# Goal reached
\# Dynamic window
v\_lo = max(self.min\_v, self.cur\_v - self.max\_accel * self.dt)
v\_hi = min(self.max\_v, self.cur\_v + self.max\_accel * self.dt)
w\_lo = max(-self.max\_w, self.cur\_w - self.max\_w\_accel * self.dt)
w\_hi = min(self.max\_w, self.cur\_w + self.max\_w\_accel * self.dt)
\# Sample and score
best\_v, best\_w, best\_score = 0.0, 0.0, -float(\textquotesingle inf\textquotesingle)
v = v\_lo
while v \textless= v\_hi:
w = w\_lo
while w \textless= w\_hi:
traj, fth = self.predict\_trajectory(v, w)
s = self.score\_trajectory(traj, fth)
if s \textgreater{} best\_score:
best\_score, best\_v, best\_w = s, v, w
w += self.w\_res
v += self.v\_res
\# Publish best command
cmd = Twist()
cmd.linear.x = best\_v; cmd.angular.z = best\_w
...
\textbf{Tuning Guide:} The weights \textbackslash(\textbackslash alpha, \textbackslash beta, \textbackslash gamma\textbackslash) dramatically affect robot behavior.
High \textbackslash(\textbackslash alpha\textbackslash) = aggressive goal-seeking (may clip obstacles).
High \textbackslash(\textbackslash beta\textbackslash) = overly cautious (slow progress).
High \textbackslash(\textbackslash gamma\textbackslash) = fast but may overshoot.
Start with \textbackslash(\textbackslash alpha = 1.0, \textbackslash beta = 1.0, \textbackslash gamma = 0.5\textbackslash) and adjust.

\subsection{Section 5: Part 3 --- AMCL: State Estimation for Control}\label{section-5-part-3-amcl-state-estimation-for-control}}

\subsubsection{5.1 Connection to Nonlinear Optimization (Week 8)}\label{connection-to-nonlinear-optimization-week-8}}

Week 8 developed \textbf{nonlinear control optimization for robotic systems}: iLQR/NMPC for trajectory optimization, and per-controller optimization of SMC / adaptive / robust / backstepping gains. Every controller in that framework assumes access to the state \textbackslash(x\textbackslash) --- but in a real robot the state must be \emph{estimated} from sensors. Controller-plus-estimator is the universal structure:

\paragraph{Week 8 NMPC / iLQR}\label{week-8-nmpc-ilqr}}
...
\item
  \textbf{State:} full nonlinear state \textbackslash(x\textbackslash in\textbackslash mathbb R\^{}n\textbackslash)
\item
  \textbf{Sensors:} any; noise model enters the estimator design, not the controller
\end{itemize}

\paragraph{Nav2 Navigation}\label{nav2-navigation}}
...
\item
  \textbf{State:} Robot pose \textbackslash((x, y, \textbackslash theta)\textbackslash) on a map
\item
...
\subsubsection{5.2 AMCL Algorithm}\label{amcl-algorithm}}

AMCL represents the belief over the robot\textquotesingle s pose as a set of weighted particles:

\textbf{Particle Set:} \textbackslash( \textbackslash mathcal\{X\} = \textbackslash\{(x\_i, y\_i, \textbackslash theta\_i, w\_i)\textbackslash\}\_\{i=1\}\^{}\{N\} \textbackslash)

where each particle \textbackslash(i\textbackslash) has pose \textbackslash((x\_i, y\_i, \textbackslash theta\_i)\textbackslash) and weight \textbackslash(w\_i\textbackslash) with \textbackslash(\textbackslash sum\_\{i=1\}\^{}\{N\} w\_i = 1\textbackslash).
...
\item
  ...
  \textbackslash{[} x\_i\textquotesingle{} = x\_i + \textbackslash delta\_\{\textbackslash text\{trans\}\} \textbackslash cos(\textbackslash theta\_i + \textbackslash delta\_\{\textbackslash text\{rot1\}\}) + \textbackslash epsilon\_x \textbackslash{]}
  \textbackslash{[} y\_i\textquotesingle{} = y\_i + \textbackslash delta\_\{\textbackslash text\{trans\}\} \textbackslash sin(\textbackslash theta\_i + \textbackslash delta\_\{\textbackslash text\{rot1\}\}) + \textbackslash epsilon\_y \textbackslash{]}
  \textbackslash{[} \textbackslash theta\_i\textquotesingle{} = \textbackslash theta\_i + \textbackslash delta\_\{\textbackslash text\{rot1\}\} + \textbackslash delta\_\{\textbackslash text\{rot2\}\} + \textbackslash epsilon\_\textbackslash theta \textbackslash{]}
  where \textbackslash(\textbackslash epsilon\textbackslash) terms are sampled from noise distributions parameterised by odometry noise coefficients \textbackslash((\textbackslash alpha\_1, \textbackslash alpha\_2, \textbackslash alpha\_3, \textbackslash alpha\_4)\textbackslash).
\item
  ...
  \textbackslash{[} w\_i = \textbackslash prod\_\{j \textbackslash in \textbackslash text\{beams\}\} \textbackslash bigl{[} z\_\{\textbackslash text\{hit\}\} \textbackslash cdot \textbackslash mathcal\{N\}(d\_j; 0, \textbackslash sigma\^{}2) + z\_\{\textbackslash text\{random\}\} \textbackslash cdot \textbackslash tfrac\{1\}\{z\_\{\textbackslash max\}\} \textbackslash bigr{]} \textbackslash{]}
  where \textbackslash(d\_j\textbackslash) is the distance from the beam endpoint to the nearest obstacle in the map.
\item
  \textbf{Normalisation:} \textbackslash( w\_i \textbackslash leftarrow w\_i / \textbackslash sum\_\{k=1\}\^{}\{N\} w\_k \textbackslash)
\item
  \textbf{Resampling:} When the effective particle count \textbackslash(N\_\{\textbackslash text\{eff\}\} = 1/\textbackslash sum w\_i\^{}2\textbackslash) drops below a threshold, perform low-variance resampling: draw a new particle set where particles with higher weights are more likely to be selected.
\end{enumerate}

\subsubsection{5.3 AMCL Cycle Diagram}\label{amcl-cycle-diagram}}

\subsubsection{5.4 ROS2 AMCL Configuration}\label{ros2-amcl-configuration}}

\# Launch AMCL for TurtleBot3 localization
ros2 launch nav2\_bringup localization\_launch.py \textbackslash{}
map:=/path/to/map.yaml \textbackslash{}
params\_file:=/path/to/nav2\_params.yaml

Key AMCL parameters in \texttt{nav2\_params.yaml}:

AMCL Parameters (nav2\_params.yaml)
...
\subsection{Section 6: Part 4 --- Costmap: Constraints for Control}\label{section-6-part-4-costmap-constraints-for-control}}
...
\subsubsection{6.1 Costmaps as Control Constraints}\label{costmaps-as-control-constraints}}

\paragraph{Global Costmap ↔ Terminal Constraint}\label{global-costmap-terminal-constraint}}
...
\paragraph{Local Costmap ↔ Stage Constraints}\label{local-costmap-stage-constraints}}

The local costmap constrains the DWA/MPPI local controller, analogous to \textbf{stage constraints} in MPC \textbackslash((\textbackslash mathbf\{x\}\_k \textbackslash in \textbackslash mathcal\{X\}, \textbackslash mathbf\{u\}\_k \textbackslash in \textbackslash mathcal\{U\})\textbackslash) that enforce safety at each time step. Rolling window, updated at 5--20 Hz.

\paragraph{Inflation Layer ↔ Soft Constraints}\label{inflation-layer-soft-constraints}}
...
\subsubsection{6.2 Costmap Layers}\label{costmap-layers}}
...
\subsubsection{6.3 Inflation Cost Function}\label{inflation-cost-function}}

\textbackslash{[}
c(d) = \textbackslash begin\{cases\}
254 \textbackslash;\textbackslash text\{(LETHAL)\} \& \textbackslash text\{if \} d \textless{} r\_\{\textbackslash text\{inscribed\}\} \textbackslash\textbackslash{[}6pt{]}
A \textbackslash cdot e\^{}\{-\textbackslash beta \textbackslash,(d - r\_\{\textbackslash text\{inscribed\}\})\} \& \textbackslash text\{if \} r\_\{\textbackslash text\{inscribed\}\} \textbackslash leq d \textbackslash leq r\_\{\textbackslash text\{inflation\}\} \textbackslash\textbackslash{[}6pt{]}
0 \textbackslash;\textbackslash text\{(FREE)\} \& \textbackslash text\{if \} d \textgreater{} r\_\{\textbackslash text\{inflation\}\}
...
\tightlist
\item
  \textbackslash(d\textbackslash) = distance from the cell to the nearest obstacle
\item
  \textbackslash(r\_\{\textbackslash text\{inscribed\}\}\textbackslash) = inscribed radius of the robot footprint (if the robot centre is closer than this, collision is guaranteed)
\item
  \textbackslash(r\_\{\textbackslash text\{inflation\}\}\textbackslash) = outer inflation radius (beyond this, cost is zero)
\item
  \textbackslash(A = 253\textbackslash) (INSCRIBED cost value, just below LETHAL)
\item
  \textbackslash(\textbackslash beta\textbackslash) = cost scaling factor controlling how quickly cost decays with distance
\end{itemize}

\subsubsection{6.4 Nav2 Costmap Configuration}\label{nav2-costmap-configuration}}

Costmap YAML Configuration (nav2\_params.yaml)
...
\subsection{Section 7: Part 5 --- Behavior Trees for Mission Control}\label{section-7-part-5-behavior-trees-for-mission-control}}

\subsubsection{7.1 Why Behavior Trees?}\label{why-behavior-trees}}
...
\subsubsection{7.2 BT Node Types}\label{bt-node-types}}
...
\subsubsection{7.3 Navigation Behavior Tree Diagram}\label{navigation-behavior-tree-diagram}}

\subsubsection{7.4 Nav2 BT XML Configuration}\label{nav2-bt-xml-configuration}}

Default Nav2 Behavior Tree XML (navigate\_to\_pose\_w\_replanning.xml)
...
\subsection{Section 8: Part 6 --- Applying Control Theory to Navigation}\label{section-8-part-6-applying-control-theory-to-navigation}}
...
\subsubsection{Detailed Mapping}\label{detailed-mapping}}

\paragraph{PID → Pure Pursuit Path Follower}\label{pid-pure-pursuit-path-follower}}
...
\paragraph{Computed Torque → Model-Based Tracking}\label{computed-torque-model-based-tracking}}

When the robot\textquotesingle s kinematic or dynamic model is well-known (mass, inertia, wheel-ground friction), computed torque control cancels the nonlinear dynamics and applies a linear outer loop. For a differential-drive robot, this means compensating for the nonholonomic constraint \textbackslash(\textbackslash dot\{x\}\textbackslash sin\textbackslash theta - \textbackslash dot\{y\}\textbackslash cos\textbackslash theta = 0\textbackslash) directly in the control law.

\paragraph{MPC → MPPI Local Planner}\label{mpc-mppi-local-planner}}

Nav2\textquotesingle s MPPI (Model Predictive Path Integral) controller is a direct implementation of MPC using sampling-based optimization. It rolls out thousands of trajectories over a prediction horizon \textbackslash(N\textbackslash), evaluates a cost function (path following + obstacle avoidance + smoothness), and selects the optimal control sequence. This is the most capable Nav2 controller for complex environments.

\paragraph{Adaptive → Terrain-Adaptive Navigation}\label{adaptive-terrain-adaptive-navigation}}
...
\paragraph{Robust (H-infinity) → All-Weather Navigation}\label{robust-h-infinity-all-weather-navigation}}
...
\paragraph{Backstepping → Nonholonomic Path Tracking}\label{backstepping-nonholonomic-path-tracking}}
...
\paragraph{SMC → Rough Terrain Tracking}\label{smc-rough-terrain-tracking}}
...
\paragraph{LQR → Highway/Corridor Following}\label{lqr-highwaycorridor-following}}
...
\subsection{Section 9: Part 7 --- ROS2 Lab Exercises}\label{section-9-part-7-ros2-lab-exercises}}
...
\subsubsection{9.1 Python Lab Exercises (Algorithmic Foundations)}\label{python-lab-exercises-algorithmic-foundations}}
...
\subsubsection{9.2 ROS2 Applied Exercises (Node-Based)}\label{ros2-applied-exercises-node-based}}
...
\textbf{Exercise 3: Path Follower (Pure Pursuit)} & \texttt{exercise3\_node.py} & \texttt{/planned\_path}, \texttt{/odom} & \texttt{/cmd\_vel} (Twist) & \texttt{pure\_pursuit()} --- Curvature: \textbackslash(\textbackslash kappa = 2 y\_\{\textbackslash text\{local\}\} / L\^{}2\textbackslash), then \textbackslash(\textbackslash omega = \textbackslash kappa \textbackslash cdot v\textbackslash) \\
\end{longtable}

\# Running the ROS2 exercises
\# Terminal 1: Play synthetic navigation bag data
ros2 bag play week\_05/w05\_exercises/bag\_data/synthetic\_nav2/
\# Terminal 2: Run the costmap generator
ros2 run week\_05 exercise1\_node -\/-ros-args \textbackslash{}
-\/-params-file week\_05/w05\_exercises/config/params.yaml
\# Terminal 3: Run the path planner
ros2 run week\_05 exercise2\_node -\/-ros-args \textbackslash{}
-\/-params-file week\_05/w05\_exercises/config/params.yaml
\# Terminal 4: Run the path follower
ros2 run week\_05 exercise3\_node -\/-ros-args \textbackslash{}
-\/-params-file week\_05/w05\_exercises/config/params.yaml

\subsection{Section 10: Hands-On Activities}\label{section-10-hands-on-activities}}

\subsubsection{Activity 1: Configure Nav2 for TurtleBot3 Navigation}\label{activity-1-configure-nav2-for-turtlebot3-navigation}}
...
\item
  ...
  ros2 launch nav2\_bringup navigation\_launch.py \textbackslash{}
  params\_file:=nav2\_params.yaml
  \# Terminal 3: Launch RViz2 with Nav2 config
  ros2 launch nav2\_bringup rviz\_launch.py
\item
...
\item
  ...
  \item
    Increase \texttt{PathAlign.scale} (the \textbackslash(\textbackslash alpha\textbackslash) heading weight) --- does the robot follow the global path more closely?
  \item
    Increase \texttt{GoalDist.scale} --- does the robot approach the goal more aggressively?
  \item
    Increase \texttt{ObstacleFootprint.scale} (the \textbackslash(\textbackslash beta\textbackslash) clearance weight) --- does the robot maintain more distance from walls?
  \end{itemize}
\item
...
\subsubsection{Activity 2: Compare DWA vs MPC for Navigation}\label{activity-2-compare-dwa-vs-mpc-for-navigation}}
...
\item
  ...
  \textbackslash{[} \textbackslash dot\{x\} = v\textbackslash cos\textbackslash theta, \textbackslash quad \textbackslash dot\{y\} = v\textbackslash sin\textbackslash theta, \textbackslash quad \textbackslash dot\{\textbackslash theta\} = \textbackslash omega \textbackslash{]}
\item
...
\item
  ...
  ros2 bag record /cmd\_vel /odom /plan /local\_costmap/costmap -o dwa\_corridor
  ros2 bag record /cmd\_vel /odom /plan /local\_costmap/costmap -o mpc\_corridor
  \# Plot trajectories for comparison
  python3 plot\_comparison.py dwa\_corridor mpc\_corridor
\item
...
\subsubsection{Activity 3: Waypoint Navigation with Recovery Behaviors}\label{activity-3-waypoint-navigation-with-recovery-behaviors}}
...
\item
  ...
  def \_\_init\_\_(self):
  super().\_\_init\_\_(\textquotesingle waypoint\_navigator\textquotesingle)
  self.nav\_client = ActionClient(self, NavigateToPose, \textquotesingle navigate\_to\_pose\textquotesingle)
  self.waypoints = {[}
  ...
  self.current\_wp = 0
  self.max\_retries = 3
  self.nav\_client.wait\_for\_server()
  self.get\_logger().info(\textquotesingle Nav2 server ready. Starting mission.\textquotesingle)
  self.navigate\_to\_next()
  def create\_goal(self, x, y, yaw):
  goal = NavigateToPose.Goal()
  goal.pose = PoseStamped()
  goal.pose.header.frame\_id = \textquotesingle map\textquotesingle{}
  goal.pose.header.stamp = self.get\_clock().now().to\_msg()
  goal.pose.pose.position.x = x
  goal.pose.pose.position.y = y
  goal.pose.pose.orientation.z = \_\_import\_\_(\textquotesingle math\textquotesingle).sin(yaw / 2)
  goal.pose.pose.orientation.w = \_\_import\_\_(\textquotesingle math\textquotesingle).cos(yaw / 2)
  return goal
  def navigate\_to\_next(self):
  if self.current\_wp \textgreater= len(self.waypoints):
  self.get\_logger().info(\textquotesingle All waypoints reached! Mission complete.\textquotesingle)
  return
  x, y, yaw = self.waypoints{[}self.current\_wp{]}
  self.get\_logger().info(f\textquotesingle Navigating to WP\{self.current\_wp\}: (\{x\}, \{y\})\textquotesingle)
  goal = self.create\_goal(x, y, yaw)
  future = self.nav\_client.send\_goal\_async(goal)
  ...
  self.get\_logger().warn(\textquotesingle Goal rejected!\textquotesingle)
  return
  result\_future = goal\_handle.get\_result\_async()
  result\_future.add\_done\_callback(self.result\_cb)
  def result\_cb(self, future):
  result = future.result()
  self.get\_logger().info(f\textquotesingle WP\{self.current\_wp\} result: \{result.status\}\textquotesingle)
  self.current\_wp += 1
  self.navigate\_to\_next()
  def main():
  rclpy.init()
...
\paragraph{Quick Quiz --- Navigation and Mobile Robot Control}\label{quick-quiz-navigation-and-mobile-robot-control}}

\textbf{Q1:} In the DWA objective function \textbackslash(G(v,\textbackslash omega) = \textbackslash alpha \textbackslash cdot \textbackslash text\{heading\} + \textbackslash beta \textbackslash cdot \textbackslash text\{clearance\} + \textbackslash gamma \textbackslash cdot \textbackslash text\{velocity\}\textbackslash), what happens if you set \textbackslash(\textbackslash beta = 0\textbackslash) (zero clearance weight)?
...
\textbf{Q4:} The inflation layer uses \textbackslash(c(d) = A \textbackslash cdot e\^{}\{-\textbackslash beta(d - r\_\{\textbackslash text\{inscribed\}\})\}\textbackslash). If you double the cost\_scaling\_factor \textbackslash(\textbackslash beta\textbackslash), what happens to the robot\textquotesingle s navigation behavior?

\paragraph{Answer}\label{answer-3}}

Doubling \textbackslash(\textbackslash beta\textbackslash) makes the exponential decay steeper, meaning the cost drops to zero much faster with distance from obstacles. The effective "buffer zone" around obstacles becomes narrower. The robot will navigate closer to walls and obstacles because the cost penalty for proximity is confined to a smaller region. This is useful in tight spaces but increases collision risk. Conversely, halving \textbackslash(\textbackslash beta\textbackslash) creates wider buffer zones and more cautious navigation.
...
\paragraph{DWA = Simplified MPC}\label{dwa-simplified-mpc}}

The Dynamic Window Approach is fundamentally a 1-step MPC: sample velocities in a dynamic window, forward-simulate, score trajectories with \textbackslash(\textbackslash alpha \textbackslash cdot h + \textbackslash beta \textbackslash cdot c + \textbackslash gamma \textbackslash cdot v\textbackslash), and select the best. Nav2\textquotesingle s MPPI extends this to full N-step MPC.

\paragraph{AMCL = State Estimator}\label{amcl-state-estimator}}
...
\paragraph{Costmaps = Constraints}\label{costmaps-constraints}}
...
\paragraph{Behavior Trees = Mission Control}\label{behavior-trees-mission-control}}
...
\paragraph{All Controllers Apply}\label{all-controllers-apply}}
...
colcon build -\/-packages-select week\_05\_nav2
source install/setup.bash

\subsubsection{Launch the Demo}\label{launch-the-demo}}

ros2 launch week\_05\_nav2 demo.launch.py
...
\subsection{Appendix: Review Material --- Quadrotor Control}\label{appendix-review-material-quadrotor-control}}
...
\tightlist
\item
  \textbf{Position Loop (Outer, \textasciitilde50 Hz):} Compares desired position \textbackslash((x\_d, y\_d, z\_d)\textbackslash) with measured position from GPS/mocap. Outputs desired roll, pitch, and thrust commands. Typically a PID or LQR controller.
\item
  \textbf{Attitude Loop (\textasciitilde250 Hz):} Compares desired roll/pitch/yaw \textbackslash((\textbackslash phi\_d, \textbackslash theta\_d, \textbackslash psi\_d)\textbackslash) with IMU-measured Euler angles. Outputs desired angular rates \textbackslash((p\_d, q\_d, r\_d)\textbackslash). Usually PID or quaternion-based control.
\item
  \textbf{Rate Loop (Inner, \textasciitilde500--1000 Hz):} Compares desired angular rates with gyroscope measurements. Outputs torque commands \textbackslash((\textbackslash tau\_x, \textbackslash tau\_y, \textbackslash tau\_z)\textbackslash). Fast PID controller critical for stability.
\item
  \textbf{Motor Mixing (\textasciitilde1000 Hz):} Converts thrust + torques into individual motor PWM signals using the allocation matrix \textbackslash(\textbackslash mathbf\{M\}\textbackslash).
\end{enumerate}
...
\tightlist
\item
  \textbf{MPC:} Explicitly minimises control effort \textbackslash(\textbackslash sum u\^{}T R u\textbackslash) in the cost function while respecting actuator constraints. Smoothest motor commands, least wasted energy.
\item
  \textbf{LQR:} Minimises \textbackslash(\textbackslash int (x\^{}T Q x + u\^{}T R u)\textbackslash, dt\textbackslash) for the linear system. Near-optimal energy usage on straight segments, but suboptimal at corners where the linearisation breaks down.
\item
