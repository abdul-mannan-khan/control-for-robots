...
\chapter{Model Predictive Control --- Design and Simulation}
\labch{week_03}

\section{Week 3: Model Predictive Control (MPC)}\label{week-3-model-predictive-control-mpc}}
...
At the operating point \textbackslash(x\_0 = {[}0, 0, 0, 0{]}\^{}T\textbackslash), \textbackslash(u\_0 = G(q\_0)\textbackslash) (the torque needed to hold the arm at \textbackslash(q\_0\textbackslash)):
...
Since \textbackslash(C(q, 0) = 0\textbackslash) at zero velocity, and the gravity linearisation yields stiffness terms, the linearised continuous-time system is:

\textbackslash{[}
A\_c = \textbackslash begin\{bmatrix\}
0 \& 0 \& 1 \& 0 \textbackslash\textbackslash{}
0 \& 0 \& 0 \& 1 \textbackslash\textbackslash{}
0 \& 0 \& 0 \& 0 \textbackslash\textbackslash{}
0 \& 0 \& 0 \& 0
\textbackslash end\{bmatrix\}, \textbackslash quad
B\_c = \textbackslash begin\{bmatrix\}
0\_\{2 \textbackslash times 2\} \textbackslash\textbackslash{}
M\^{}\{-1\}(q\_0)
\textbackslash end\{bmatrix\}
\textbackslash{]}

Computing \textbackslash(M\^{}\{-1\}(0)\textbackslash): since \textbackslash(\textbackslash det(M(0)) = 4 \textbackslash cdot 1 - 2 \textbackslash cdot 2 = 0\textbackslash)... Note that \textbackslash(M(0)\textbackslash) is singular at this particular configuration because we used point masses at link tips. In practice we use the more general parametrisation with \textbackslash(l\_\{c1\}, l\_\{c2\}\textbackslash). For our MATLAB implementation, we evaluate at a slightly offset configuration \textbackslash(q\_0 = {[}0.1, 0.1{]}\^{}T\textbackslash) to ensure non-singularity, giving:
...
Using the zero-order hold (ZOH) method with sampling time \textbackslash(\textbackslash Delta t\textbackslash):
...
First-order Euler approximation, valid for small \textbackslash(\textbackslash Delta t\textbackslash).

\subsubsection{2.4 MPC Design for the Manipulator}\label{mpc-design-for-the-manipulator}}

State: \textbackslash(x = {[}q\_1, q\_2, \textbackslash dot\{q\}\_1, \textbackslash dot\{q\}\_2{]}\^{}T\textbackslash), Control: \textbackslash(u = {[}\textbackslash tau\_1, \textbackslash tau\_2{]}\^{}T\textbackslash)
...
The ratio \textbackslash(Q/R\textbackslash) determines aggressiveness: here \textbackslash(q\_\{ii\}/r\_\{ii\} = 10\{,\}000\textbackslash), so the controller strongly prioritises position tracking over control effort economy.

With prediction horizon \textbackslash(N = 20\textbackslash) and \textbackslash(\textbackslash Delta t = 0.01\textbackslash,\textbackslash text\{s\}\textbackslash), we build the prediction matrices \textbackslash(\textbackslash mathcal\{A\} \textbackslash in \textbackslash mathbb\{R\}\^{}\{80 \textbackslash times 4\}\textbackslash) and \textbackslash(\textbackslash mathcal\{B\} \textbackslash in \textbackslash mathbb\{R\}\^{}\{80 \textbackslash times 40\}\textbackslash),
then compute the Hessian \textbackslash(H = \textbackslash mathcal\{B\}\^{}T \textbackslash mathbf\{Q\}\_N \textbackslash mathcal\{B\} + \textbackslash mathbf\{R\}\_N \textbackslash in \textbackslash mathbb\{R\}\^{}\{40 \textbackslash times 40\}\textbackslash).
...
A differential-drive mobile robot (e.g., TurtleBot) moves in the plane with state \textbackslash(x = {[}p\_x, p\_y, \textbackslash theta{]}\^{}T\textbackslash) (position and heading) and control inputs \textbackslash(u = {[}v, \textbackslash omega{]}\^{}T\textbackslash) (linear and angular velocity):
...
Discretised with Euler integration at sampling time \textbackslash(\textbackslash Delta t\textbackslash):

\textbackslash{[}
\textbackslash begin\{aligned\}
p\_\{x,k+1\} \&= p\_\{x,k\} + v\_k \textbackslash cos(\textbackslash theta\_k)\textbackslash,\textbackslash Delta t \textbackslash\textbackslash{}
p\_\{y,k+1\} \&= p\_\{y,k\} + v\_k \textbackslash sin(\textbackslash theta\_k)\textbackslash,\textbackslash Delta t \textbackslash\textbackslash{}
\textbackslash theta\_\{k+1\} \&= \textbackslash theta\_k + \textbackslash omega\_k\textbackslash,\textbackslash Delta t
...
Let the reference trajectory be \textbackslash((p\_x\^{}r(t), p\_y\^{}r(t), \textbackslash theta\^{}r(t))\textbackslash) with reference controls \textbackslash((v\^{}r(t), \textbackslash omega\^{}r(t))\textbackslash).
Define error variables:

\textbackslash{[}
\textbackslash delta x = x - x\^{}r, \textbackslash quad \textbackslash delta u = u - u\^{}r
\textbackslash{]}

The Jacobians of the continuous-time model at the reference are:

\textbackslash{[}
A\_c = \textbackslash frac\{\textbackslash partial f\}\{\textbackslash partial x\}\textbackslash bigg\textbar\_\{x\^{}r, u\^{}r\} = \textbackslash begin\{bmatrix\}
0 \& 0 \& -v\^{}r \textbackslash sin\textbackslash theta\^{}r \textbackslash\textbackslash{}
0 \& 0 \& v\^{}r \textbackslash cos\textbackslash theta\^{}r \textbackslash\textbackslash{}
0 \& 0 \& 0
\textbackslash end\{bmatrix\}
\textbackslash{]}
\textbackslash{[}
B\_c = \textbackslash frac\{\textbackslash partial f\}\{\textbackslash partial u\}\textbackslash bigg\textbar\_\{x\^{}r, u\^{}r\} = \textbackslash begin\{bmatrix\}
\textbackslash cos\textbackslash theta\^{}r \& 0 \textbackslash\textbackslash{}
\textbackslash sin\textbackslash theta\^{}r \& 0 \textbackslash\textbackslash{}
0 \& 1
\textbackslash end\{bmatrix\}
\textbackslash{]}

For simplicity, we linearise around \textbackslash(\textbackslash theta\^{}r = 0\textbackslash) (heading east) and \textbackslash(v\^{}r = 2.5\textbackslash,\textbackslash text\{m/s\}\textbackslash):
...
\textbackslash{[}
A\_d = I + A\_c \textbackslash Delta t, \textbackslash quad B\_d = B\_c \textbackslash Delta t
\textbackslash{]}
...
\bottomrule\noalign{}
\endlastfoot
\textbackslash(0 \textbackslash leq v \textbackslash leq 5\textbackslash,\textbackslash text\{m/s\}\textbackslash) & The robot cannot move backwards (for safety) and has a maximum speed & {[}0, 5{]} \\
\textbackslash(-2 \textbackslash leq \textbackslash omega \textbackslash leq 2\textbackslash,\textbackslash text\{rad/s\}\textbackslash) & Maximum turning rate limited by wheel motors & {[}-2, 2{]} \\
\end{longtable}

\subsubsection{3.4 MPC Design}\label{mpc-design}}

State: \textbackslash(x = {[}p\_x, p\_y, \textbackslash theta{]}\^{}T\textbackslash), Control: \textbackslash(u = {[}v, \textbackslash omega{]}\^{}T\textbackslash), Horizon: \textbackslash(N = 15\textbackslash), Weights:

\textbackslash{[} Q = \textbackslash text\{diag\}(50, 50, 10), \textbackslash quad R = \textbackslash text\{diag\}(0.1, 0.1) \textbackslash{]}

The higher weight on position relative to heading ensures the robot prioritises reaching the correct \textbackslash((x, y)\textbackslash) location, with heading as a secondary objective.
...
v\_ref = 2.5; \% Reference linear velocity
dt = 0.05; \% Sampling time
\% Linearised system (around theta = 0, v = v\_ref)
...
x\_mob = {[}0; 0; 0{]};
x\_hist\_mob = zeros(3, length(t\_span\_mob));
u\_hist\_mob = zeros(2, length(t\_span\_mob));
x\_hist\_mob(:,1) = x\_mob;
...
\% Enforce constraints
u\_mob(1) = max(min(u\_mob(1), 5), 0); \% v in {[}0, 5{]}
u\_mob(2) = max(min(u\_mob(2), 2), -2); \% omega in {[}-2, 2{]}
\% True nonlinear dynamics (Euler integration)
theta = x\_mob(3);
x\_mob = x\_mob + {[}u\_mob(1)*cos(theta); u\_mob(1)*sin(theta); u\_mob(2){]}*dt;
x\_hist\_mob(:,k+1) = x\_mob;
u\_hist\_mob(:,k) = u\_mob;
end

\textbf{Linearisation Caveat:} The linearisation is performed at a single operating point. When the robot heading \textbackslash(\textbackslash theta\textbackslash) deviates significantly from \textbackslash(\textbackslash theta\^{}r = 0\textbackslash), the linear model becomes inaccurate. In practice, one re-linearises at each time step (forming a time-varying linear MPC), or uses nonlinear MPC.
...
\subsubsection{4.2 Nonlinear Equations of Motion}\label{nonlinear-equations-of-motion}}

\paragraph{Full Quadrotor Dynamics}\label{full-quadrotor-dynamics}}

\textbf{Translational dynamics} (Newton\textquotesingle s second law in the world frame):

\textbackslash{[}
\textbackslash begin\{aligned\}
\textbackslash dot\{v\}\_x \&= \textbackslash frac\{T\}\{m\}\textbackslash bigl(\textbackslash cos\textbackslash phi\textbackslash sin\textbackslash theta\textbackslash cos\textbackslash psi + \textbackslash sin\textbackslash phi\textbackslash sin\textbackslash psi\textbackslash bigr) \textbackslash\textbackslash{}
\textbackslash dot\{v\}\_y \&= \textbackslash frac\{T\}\{m\}\textbackslash bigl(\textbackslash cos\textbackslash phi\textbackslash sin\textbackslash theta\textbackslash sin\textbackslash psi - \textbackslash sin\textbackslash phi\textbackslash cos\textbackslash psi\textbackslash bigr) \textbackslash\textbackslash{}
\textbackslash dot\{v\}\_z \&= \textbackslash frac\{T\}\{m\}\textbackslash cos\textbackslash phi\textbackslash cos\textbackslash theta - g
\textbackslash end\{aligned\}
\textbackslash{]}
...
\textbackslash{[}
\textbackslash begin\{aligned\}
\textbackslash dot\{\textbackslash omega\}\_x \&= \textbackslash frac\{\textbackslash tau\_x\}\{I\_\{xx\}\} + \textbackslash frac\{(I\_\{yy\} - I\_\{zz\})\}\{I\_\{xx\}\}\textbackslash omega\_y\textbackslash omega\_z \textbackslash\textbackslash{}
\textbackslash dot\{\textbackslash omega\}\_y \&= \textbackslash frac\{\textbackslash tau\_y\}\{I\_\{yy\}\} + \textbackslash frac\{(I\_\{zz\} - I\_\{xx\})\}\{I\_\{yy\}\}\textbackslash omega\_x\textbackslash omega\_z \textbackslash\textbackslash{}
\textbackslash dot\{\textbackslash omega\}\_z \&= \textbackslash frac\{\textbackslash tau\_z\}\{I\_\{zz\}\} + \textbackslash frac\{(I\_\{xx\} - I\_\{yy\})\}\{I\_\{zz\}\}\textbackslash omega\_x\textbackslash omega\_y
\textbackslash end\{aligned\}
\textbackslash{]}

\subsubsection{4.3 Linearisation around Hover}\label{linearisation-around-hover}}

At hover, all velocities and angular rates are zero, \textbackslash(\textbackslash phi = \textbackslash theta = \textbackslash psi = 0\textbackslash), and the thrust equals gravity: \textbackslash(T\_0 = mg\textbackslash).
...
\paragraph{Linearised Hover Dynamics}\label{linearised-hover-dynamics}}
...
\textbackslash{[}
\textbackslash dot\{v\}\_x \textbackslash approx g\textbackslash,\textbackslash theta, \textbackslash quad
\textbackslash dot\{v\}\_y \textbackslash approx -g\textbackslash,\textbackslash phi, \textbackslash quad
\textbackslash dot\{v\}\_z \textbackslash approx \textbackslash frac\{T - mg\}\{m\}
\textbackslash{]}
\textbackslash{[}
\textbackslash dot\{\textbackslash omega\}\_x \textbackslash approx \textbackslash frac\{\textbackslash tau\_x\}\{I\_\{xx\}\}, \textbackslash quad
\textbackslash dot\{\textbackslash omega\}\_y \textbackslash approx \textbackslash frac\{\textbackslash tau\_y\}\{I\_\{yy\}\}, \textbackslash quad
\textbackslash dot\{\textbackslash omega\}\_z \textbackslash approx \textbackslash frac\{\textbackslash tau\_z\}\{I\_\{zz\}\}
\textbackslash{]}
...
\textbackslash{[}
A\_c = \textbackslash begin\{bmatrix\}
0\_3 \& I\_3 \& 0\_3 \& 0\_3 \textbackslash\textbackslash{}
0\_3 \& 0\_3 \& G\_\{\textbackslash text\{lin\}\} \& 0\_3 \textbackslash\textbackslash{}
0\_3 \& 0\_3 \& 0\_3 \& I\_3 \textbackslash\textbackslash{}
0\_3 \& 0\_3 \& 0\_3 \& 0\_3
\textbackslash end\{bmatrix\}, \textbackslash quad
B\_c = \textbackslash begin\{bmatrix\}
0\_3 \textbackslash\textbackslash{} B\_T \textbackslash\textbackslash{} 0\_3 \textbackslash\textbackslash{} B\_\textbackslash tau
\textbackslash end\{bmatrix\}
\textbackslash{]}

where the coupling matrix \textbackslash(G\_\{\textbackslash text\{lin\}\}\textbackslash) and input matrices are:

\textbackslash{[}
G\_\{\textbackslash text\{lin\}\} = \textbackslash begin\{bmatrix\} 0 \& g \& 0 \textbackslash\textbackslash{} -g \& 0 \& 0 \textbackslash\textbackslash{} 0 \& 0 \& 0 \textbackslash end\{bmatrix\}, \textbackslash quad
B\_T = \textbackslash begin\{bmatrix\} 0 \textbackslash\textbackslash{} 0 \textbackslash\textbackslash{} 1/m \textbackslash end\{bmatrix\} \textbackslash begin\{bmatrix\} 1 \& 0 \& 0 \& 0 \textbackslash end\{bmatrix\}, \textbackslash quad
B\_\textbackslash tau = \textbackslash text\{diag\}\textbackslash!\textbackslash left(\textbackslash frac\{1\}\{I\_\{xx\}\}, \textbackslash frac\{1\}\{I\_\{yy\}\}, \textbackslash frac\{1\}\{I\_\{zz\}\}\textbackslash right) \textbackslash begin\{bmatrix\} 0 \& I\_3 \textbackslash end\{bmatrix\}
\textbackslash{]}

\subsubsection{4.4 MPC Design for the Quadrotor}\label{mpc-design-for-the-quadrotor}}

State dimension: \textbackslash(n = 12\textbackslash), Control dimension: \textbackslash(m = 4\textbackslash), Horizon: \textbackslash(N = 15\textbackslash).
...
\subsubsection{4.5 Constraints}\label{constraints-1}}
...
\bottomrule\noalign{}
\endlastfoot
\textbackslash(0 \textbackslash leq T \textbackslash leq 2mg\textbackslash) & Thrust cannot be negative; max is twice the hover thrust & {[}0, 19.62{]} N \\
\textbackslash(-2 \textbackslash leq \textbackslash tau\_x, \textbackslash tau\_y, \textbackslash tau\_z \textbackslash leq 2\textbackslash) & Maximum body torques from motor differentials & {[}-2, 2{]} N·m \\
\textbackslash(-30\^{}\textbackslash circ \textbackslash leq \textbackslash phi, \textbackslash theta \textbackslash leq 30\^{}\textbackslash circ\textbackslash) & Tilt angle limits for safety & {[}-0.524, 0.524{]} rad \\
\end{longtable}

\subsubsection{4.6 MATLAB Implementation --- Quadrotor MPC}\label{matlab-implementation-quadrotor-mpc}}
...
quad.g = 9.81;
dt\_q = 0.02; \% Sampling time (50 Hz)
n\_q = 12; m\_q = 4;
\% Linearised continuous-time matrices (hover)
Ac\_q = zeros(12,12);
Ac\_q(1:3, 4:6) = eye(3); \% dp/dt = v
Ac\_q(4,8) = quad.g; \% dvx/dt = g*theta
Ac\_q(5,7) = -quad.g; \% dvy/dt = -g*phi
Ac\_q(7:9, 10:12) = eye(3); \% datt/dt = omega
Bc\_q = zeros(12,4);
Bc\_q(6,1) = 1/quad.m; \% dvz/dt = T/m
Bc\_q(10,2) = 1/quad.Ixx; \% domega\_x/dt = tau\_x/Ixx
Bc\_q(11,3) = 1/quad.Iyy; \% domega\_y/dt = tau\_y/Iyy
Bc\_q(12,4) = 1/quad.Izz; \% domega\_z/dt = tau\_z/Izz
\% Discretise
...
Q\_N\_q = kron(eye(N\_q), Q\_q);
R\_N\_q = kron(eye(N\_q), R\_q);
H\_q = B\_cal\_q\textquotesingle{} * Q\_N\_q * B\_cal\_q + R\_N\_q;
H\_inv\_q = inv(H\_q);
F\_q = B\_cal\_q\textquotesingle{} * Q\_N\_q;
\% Simulation (helical trajectory)
t\_span\_q = 0:dt\_q:20;
x\_q = zeros(12,1);
x\_hist\_q = zeros(12, length(t\_span\_q));
u\_hist\_q = zeros(4, length(t\_span\_q));
x\_hist\_q(:,1) = x\_q;
\% Helical reference: circle in XY with linear climb in Z
R\_helix = 3; omega\_helix = 0.5; vz\_helix = 0.3;
for k = 1:length(t\_span\_q)-1
t = t\_span\_q(k);
\% Build reference over horizon
x\_ref\_q = zeros(n\_q*N\_q, 1);
for i = 1:N\_q
t\_f = t + i*dt\_q;
px\_ref = R\_helix*sin(omega\_helix*t\_f);
py\_ref = R\_helix*cos(omega\_helix*t\_f);
pz\_ref = 5 + vz\_helix*sin(0.2*t\_f);
vx\_ref = R\_helix*omega\_helix*cos(omega\_helix*t\_f);
vy\_ref = -R\_helix*omega\_helix*sin(omega\_helix*t\_f);
vz\_ref = vz\_helix*0.2*cos(0.2*t\_f);
x\_ref\_q((i-1)*n\_q+1:i*n\_q) = {[}px\_ref; py\_ref; pz\_ref; ...
vx\_ref; vy\_ref; vz\_ref; ...
0; 0; 0; 0; 0; 0{]};
end
\% MPC solve (note: u here is delta\_u around hover)
E\_q = x\_ref\_q - A\_cal\_q * x\_q;
U\_star\_q = H\_inv\_q * F\_q * E\_q;
u\_q = U\_star\_q(1:m\_q);
\% Add hover thrust offset and enforce constraints
u\_q(1) = u\_q(1) + quad.m*quad.g; \% Total thrust = hover + correction
u\_q(1) = max(min(u\_q(1), 2*quad.m*quad.g), 0);
u\_q(2:4) = max(min(u\_q(2:4), 2), -2);
\% Simulate true nonlinear dynamics (simplified Euler step)
pos = x\_q(1:3); vel = x\_q(4:6);
att = x\_q(7:9); omg = x\_q(10:12);
phi = att(1); th = att(2); psi = att(3);
T = u\_q(1); tau\_body = u\_q(2:4);
...
domg\_x = tau\_body(1)/quad.Ixx + (quad.Iyy-quad.Izz)/quad.Ixx*omg(2)*omg(3);
domg\_y = tau\_body(2)/quad.Iyy + (quad.Izz-quad.Ixx)/quad.Iyy*omg(1)*omg(3);
domg\_z = tau\_body(3)/quad.Izz + (quad.Ixx-quad.Iyy)/quad.Izz*omg(1)*omg(2);
\% Euler integration
x\_q = x\_q + {[}vel; ax; ay; az; omg; domg\_x; domg\_y; domg\_z{]}*dt\_q;
x\_hist\_q(:,k+1) = x\_q;
u\_hist\_q(:,k) = u\_q;
end
...
Run the manipulator MPC code with three different prediction horizons. For each, record the peak tracking error \textbackslash(\textbackslash max \textbar e(t)\textbar\textbackslash) and the computation time.

\begin{longtable}[]{@{}llll@{}}
\toprule\noalign{}
Horizon \textbackslash(N\textbackslash) & Peak Position Error (rad) & Steady-State Error (rad) & Approx. Computation Time \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
\tightlist
\item
  How does increasing \textbackslash(N\textbackslash) affect the transient response? Is there a point of diminishing returns?
\item
  Why does a very short horizon (\textbackslash(N = 5\textbackslash)) lead to poor performance? Relate to the "myopic" nature of a short prediction window.
\item
  At what value of \textbackslash(N\textbackslash) does the computation time become a concern for real-time control at 100 Hz?
\end{enumerate}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Model Knowledge} & Requires linear (or nonlinear) model; can use approximate models with re-linearisation & Requires full nonlinear dynamics \textbackslash(M(q)\textbackslash), \textbackslash(C(q,\textbackslash dot\{q\})\textbackslash), \textbackslash(G(q)\textbackslash) \\
...
\textbf{Computation} & Solves a QP at every time step; computational cost grows with \textbackslash(N\textbackslash), \textbackslash(n\textbackslash), \textbackslash(m\textbackslash) & Evaluates \textbackslash(M\textbackslash), \textbackslash(C\textbackslash), \textbackslash(G\textbackslash) and a matrix-vector product; very fast per step \\
...
MPC converts the control problem into a \textbf{repeated optimisation} problem. The quality of the controller depends directly on the quality of the model and the tuning of \textbackslash(N\textbackslash), \textbackslash(Q\textbackslash), \textbackslash(R\textbackslash).
