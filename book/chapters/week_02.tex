...
\chapter{Model Predictive Control --- Introduction (Week 2)}
\labch{week_02}
...
\[
                    J = \mathbf{U}^T \underset{\text{\footnotesize H}}{\underline{(\mathcal{B}^T \mathbf{Q}_N \mathcal{B} + \mathbf{R}_N)}} \mathbf{U}
...
\topruleParameter & Increase Effect & Decrease Effect \\
\midrule\bottomrulePrediction horizon \(N\) & Better anticipation of future references; more stable; higher computational cost (\(O(N^3 m^3)\)) & More myopic; may oscillate or become unstable; cheaper to compute \\
...
At the operating point \(x_0 = [0, 0, 0, 0]^T\), \(u_0 = G(q_0)\):
...
We evaluate at a slightly offset configuration \(q_0 = [0.1, 0.1]^T\) to ensure non-singularity:
...
A differential-drive mobile robot moves in the plane with state \(x = [p_x, p_y, \theta]^T\) and control inputs \(u = [v, \omega]^T\):
...
\[
                    A_c = \begin{bmatrix} 0 & 0 & -v^r \sin\theta^r \\ 0 & 0 & v^r \cos\theta^r \\ 0 & 0 & 0 \end{bmatrix}, \quad
                    B_c = \begin{bmatrix} \cos\theta^r & 0 \\ \sin\theta^r & 0 \\ 0 & 1 \end{bmatrix}
                    \]

For \(\theta^r = 0\), \(v^r = 2.5\,\text{m/s}\):
...
\topruleConstraint & Physical Meaning & Value \\
\midrule\bottomrule\(0 \leq v \leq 5\,\text{m/s}\) & No backward motion; maximum speed & {[}0, 5{]} \\
\(-2 \leq \omega \leq 2\,\text{rad/s}\) & Maximum turning rate & {[}-2, 2{]} \\
\end{tabular}}
\end{table}
...
N_mob = 15; Q_mob = diag([50, 50, 10]); R_mob = diag([0.1, 0.1]);
n_mob = 3; m_mob = 2;
...
for k = 1:length(t_span_mob)-1
    ...
    u_mob(1) = max(min(u_mob(1), 5), 0);
    u_mob(2) = max(min(u_mob(2), 2), -2);
    theta = x_mob(3);
...
\subsection{Linearised Hover Dynamics}
...
\[
                    \dot{v}_x \approx g\,\theta, \quad \dot{v}_y \approx -g\,\phi, \quad \dot{v}_z \approx \frac{T - mg}{m}
                    \]
\[
                    \dot{\omega}_x \approx \frac{\tau_x}{I_{xx}}, \quad \dot{\omega}_y \approx \frac{\tau_y}{I_{yy}}, \quad \dot{\omega}_z \approx \frac{\tau_z}{I_{zz}}
                    \]

\subsection{MPC Design for the Quadrotor}

\[
                    Q = \text{diag}(\underset{\text{\footnotesize position}}{\underline{80, 80, 120}}, \underset{\text{\footnotesize velocity}}{\underline{1, 1, 1}}, \underset{\text{\footnotesize orientation}}{\underline{50, 50, 50}}, \underset{\text{\footnotesize angular rate}}{\underline{1, 1, 1}})
                    \]
\[
...
\topruleConstraint & Physical Meaning & Value \\
\midrule\bottomrule\(0 \leq T \leq 2mg\) & Thrust range & {[}0, 19.62{]} N \\
\(-2 \leq \tau_x, \tau_y, \tau_z \leq 2\) & Maximum body torques & {[}-2, 2{]} N·m \\
\(-30^\circ \leq \phi, \theta \leq 30^\circ\) & Tilt angle limits & {[}-0.524, 0.524{]} rad \\
\end{tabular}}
\end{table}
...
dt_q = 0.02; n_q = 12; m_q = 4;

Ac_q = zeros(12,12);
Ac_q(1:3, 4:6) = eye(3);
Ac_q(4,8) = quad.g;
Ac_q(5,7) = -quad.g;
Ac_q(7:9, 10:12) = eye(3);

Bc_q = zeros(12,4);
Bc_q(6,1) = 1/quad.m;
Bc_q(10,2) = 1/quad.Ixx;
Bc_q(11,3) = 1/quad.Iyy;
Bc_q(12,4) = 1/quad.Izz;
...
H_q = B_cal_q' * kron(eye(N_q),Q_q) * B_cal_q + kron(eye(N_q),R_q);
H_inv_q = inv(H_q);
F_q = B_cal_q' * kron(eye(N_q),Q_q);
...
\topruleHorizon \(N\) & Peak Position Error & Steady-State Error & Computation Time \\
\midrule\bottomrule\(N = 5\) & {[}record{]} & {[}record{]} & {[}record{]} \\
...
\midrule\bottomrule\(x_{k+1} = A\,x_k + B\,u_k\) & Discrete-time linear system model \\
...
\topruleProperty & MPC & Computed Torque \\
\midrule\bottomrule\textbf{Model Knowledge} & Requires linear (or nonlinear) model; can use approximate models & Requires full nonlinear dynamics \(M(q)\), \(C(q,\dot{q})\), \(G(q)\) \\
