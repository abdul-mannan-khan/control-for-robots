...
\bottomrule\noalign{}
\endlastfoot
\(0 \leq v \leq 5\,\text{m/s}\) & No backward motion; maximum speed & {[}0, 5{]} \\
...
\bottomrule\noalign{}
\endlastfoot
\(0 \leq T \leq 2mg\) & Thrust range & {[}0, 19.62{]} N \\
...
\toprule\noalign{}
Horizon \(N\) & Peak Position Error & Steady-State Error & Computation Time \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Model Knowledge} & Requires linear (or nonlinear) model; can use approximate models & Requires full nonlinear dynamics \(M(q)\), \(C(q,\dot{q})\), \(G(q)\) \\
